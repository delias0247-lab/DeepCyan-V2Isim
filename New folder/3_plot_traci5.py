...
- Dual-axis plot: Cumulative reward vs Total queue (all junctions)
"""
...
def main():
        ...
        for step in range(TOTAL_STEPS):
            ...
            if step % 100 == 0:
                ...
                print(
                    ...
                    f"Q(Node1)={qs['Node1'][2]} Q(Node2)={qs['Node2'][2]} "
                    f"Q(Node3)={qs['Node3'][2]} Q(Node4)={qs['Node4'][2]}"
                )
    ...
    for i in range(len(step_history)):
        total_q = 0
        for tls in TLS_IDS:
            total_q += q_hist[tls][i]
        total_queue_all.append(total_q)
    ...
    # Right axis: total queue across all junctions
    ax2 = ax1.twinx()
    line2 = ax2.plot(
        step_history,
        total_queue_all,
    ...
    plt.title("Map5: Dual Axis — Cumulative Reward vs Total Queue")
